...
uint64_t test_generator::getAlreadyGeneratedCoins(const isocoin::Block& blk) const {
  Crypto::Hash blkHash;
  get_block_hash(blk, blkHash);
  return getAlreadyGeneratedCoins(blkHash);
}

void test_generator::addBlock(const isocoin::Block& blk, size_t tsxSize, uint64_t fee,
...
bool test_generator::constructBlock(isocoin::Block& blk, uint32_t height, const Crypto::Hash& previousBlockHash,
...
bool test_generator::constructBlock(isocoin::Block& blk, const isocoin::AccountBase& minerAcc, uint64_t timestamp) {
  std::vector<size_t> blockSizes;
  std::list<isocoin::Transaction> txList;
  return constructBlock(blk, 0, NULL_HASH, minerAcc, timestamp, 0, blockSizes, txList);
}

bool test_generator::constructBlock(isocoin::Block& blk, const isocoin::Block& blkPrev,
                                    const isocoin::AccountBase& minerAcc,
...
bool test_generator::constructBlockManuallyTx(isocoin::Block& blk, const isocoin::Block& prevBlock,
                                              const isocoin::AccountBase& minerAcc,
...
bool test_generator::constructMaxSizeBlock(isocoin::Block& blk, const isocoin::Block& blkPrev,
                                           const isocoin::AccountBase& minerAccount,
...
void fillNonce(isocoin::Block& blk, const difficulty_type& diffic) {
  blk.nonce = 0;
  Crypto::cn_context context;
...
bool constructMinerTxBySize(const isocoin::Currency& currency, isocoin::Transaction& baseTransaction, uint32_t height,
  ...
  if (!currency.constructMinerTx(height, Common::medianValue(blockSizes), alreadyGeneratedCoins, targetBlockSize,
      fee, minerAddress, baseTransaction, isocoin::BinaryArray(), 1)) {
    return false;
  }
