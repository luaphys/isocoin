...
public:
  ...
  isocoin::TransactionId waitExternalTransaction() {
    std::unique_lock<std::mutex> lk(m_mutex);
    ...
    isocoin::TransactionId txId = m_externalTransactions.front();
  ...
  std::error_code waitSendResult(isocoin::TransactionId txid) {
    std::unique_lock<std::mutex> lk(m_mutex);

    std::unordered_map<isocoin::TransactionId, std::error_code>::iterator it;
...
private:
  ...
  std::unordered_map<isocoin::TransactionId, std::error_code> m_sendResults;
  std::deque<isocoin::TransactionId> m_externalTransactions;
};
