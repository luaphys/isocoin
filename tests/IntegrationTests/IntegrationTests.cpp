...
public:
  ...
  IntegrationTest() : 
    currency(isocoin::CurrencyBuilder(log).testnet(true).currency()), 
  ...
  void makeWallets() {
    for (auto& n: inodes) {
      std::unique_ptr<isocoin::IWalletLegacy> wallet(new isocoin::WalletLegacy(m_currency, *n));
...
TEST_F(IntegrationTest, BlockPropagationSpeed) {
  ...
  {
    std::unique_ptr<isocoin::INode>& localNode = inodes.front();
    std::unique_ptr<isocoin::INode>& remoteNode = inodes.back();
