...
public:
  ...
  SimpleTest(const isocoin::Currency& currency, System::Dispatcher& system, const Tests::Common::BaseFunctionalTestsConfig& config) : 
    BaseFunctionalTests(currency, system, config) {}

  class WaitForActualGrowObserver : public isocoin::IWalletLegacyObserver {
  ...
  class WaitForActualDwindleObserver : public isocoin::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForPendingGrowObserver : public isocoin::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForConfirmationObserver : public isocoin::IWalletLegacyObserver {
  ...
  class WaitForSendCompletedObserver : public isocoin::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_Sent;
    std::error_code& m_error;
    isocoin::TransactionId& m_transactionId;

  public:
    WaitForSendCompletedObserver(Tests::Common::Semaphore& Sent, isocoin::TransactionId& transactionId, std::error_code& error) : m_Sent(Sent), m_transactionId(transactionId), m_error(error) { }
  ...
  class WaitForExternalTransactionObserver : public isocoin::IWalletLegacyObserver {
  public:
    WaitForExternalTransactionObserver() { }
    std::promise<isocoin::TransactionId> promise;
  ...
  class WaitForTransactionUpdated : public isocoin::IWalletLegacyObserver {
  public:
    ...
    isocoin::TransactionId expectindTxId;
  };
  ...
  bool perform1() {
    ...
    std::unique_ptr<isocoin::INode> node1;
    std::unique_ptr<isocoin::INode> node2;

    nodeDaemons.front()->makeINode(node1);
    nodeDaemons.front()->makeINode(node2);

    std::unique_ptr<isocoin::IWalletLegacy> wallet1;
    std::unique_ptr<isocoin::IWalletLegacy> wallet2;
  ...
  class WaitForBlockchainHeightChangeObserver : public isocoin::INodeObserver {
  ...
  class CallbackHeightChangeObserver : public isocoin::INodeObserver {
  ...
  bool perform2(size_t blocksCount = 10)
  {
    ...
    mineBlock();
    LOG_TRACE("STEP 2 PASSED");
    std::unique_ptr<isocoin::INode> localNode;
    std::unique_ptr<isocoin::INode> remoteNode;

    nodeDaemons.front()->makeINode(localNode);
    nodeDaemons.back()->makeINode(remoteNode);
    
    std::unique_ptr<isocoin::IWalletLegacy> wallet;
  ...
  bool perform4() {
    ...
    std::unique_ptr<isocoin::INode> hopNode;
    std::unique_ptr<isocoin::INode> localNode;
    std::unique_ptr<isocoin::INode> remoteNode;
  ...
  bool perform5() {
    ...
    std::unique_ptr<isocoin::INode> node1;
    std::unique_ptr<isocoin::INode> inprocNode;
...
void testMultiVersion(const isocoin::Currency& currency, System::Dispatcher& d, const Tests::Common::BaseFunctionalTestsConfig& config);
...
public:
  ...
  SimpleTestCase() : 
    currency(isocoin::CurrencyBuilder(logger).testnet(true).currency()), 
...
TEST_F(SimpleTestCase, TESTPOOLDELETION) {
  currency = isocoin::CurrencyBuilder(logger).testnet(true).mempoolTxLiveTime(60).currency();
