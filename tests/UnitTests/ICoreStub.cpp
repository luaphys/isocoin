...
ICoreStub::ICoreStub(const isocoin::Block& genesisBlock) :
    topHeight(0),
    globalIndicesResult(false),
    randomOutsResult(false),
    poolTxVerificationResult(true),
...
bool ICoreStub::addObserver(isocoin::ICoreObserver* observer) {
  return true;
}

bool ICoreStub::removeObserver(isocoin::ICoreObserver* observer) {
  return true;
}
...
bool ICoreStub::get_random_outs_for_amounts(const isocoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request& req,
    isocoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& res) {
  res = randomOuts;
...
isocoin::i_isocoin_protocol* ICoreStub::get_protocol() {
  return nullptr;
}

bool ICoreStub::handle_incoming_tx(isocoin::BinaryArray const& tx_blob, isocoin::tx_verification_context& tvc, bool keeped_by_block) {
  return true;
}
...
void ICoreStub::set_random_outs(const isocoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp, bool result) {
  randomOuts = resp;
  randomOutsResult = result;
}

std::vector<isocoin::Transaction> ICoreStub::getPoolTransactions() {
  return std::vector<isocoin::Transaction>();
}

bool ICoreStub::getPoolChanges(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds,
                               std::vector<isocoin::Transaction>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  ...
  for (const std::pair<Crypto::Hash, isocoin::Transaction>& poolEntry : transactionPool) {
...
bool ICoreStub::getPoolChangesLite(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds,
        std::vector<isocoin::TransactionPrefixInfo>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  std::vector<isocoin::Transaction> added;
  bool returnStatus = getPoolChanges(tailBlockId, knownTxsIds, added, deletedTxsIds);

  for (const auto& tx : added) {
    isocoin::TransactionPrefixInfo tpi;
    tpi.txPrefix = tx;
...
void ICoreStub::getPoolChanges(const std::vector<Crypto::Hash>& knownTxsIds, std::vector<isocoin::Transaction>& addedTxs,
                               std::vector<Crypto::Hash>& deletedTxsIds) {
}

bool ICoreStub::queryBlocks(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
  uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<isocoin::BlockFullInfo>& entries) {
  //stub
  return true;
}

bool ICoreStub::queryBlocksLite(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
  uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<isocoin::BlockShortInfo>& entries) {
  //stub
  return true;
}
...
size_t ICoreStub::addChain(const std::vector<const isocoin::IBlock*>& chain) {
  size_t blocksCounter = 0;
  for (const isocoin::IBlock* block : chain) {
    for (size_t txNumber = 0; txNumber < block->getTransactionCount(); ++txNumber) {
      const isocoin::Transaction& tx = block->getTransaction(txNumber);
      Crypto::Hash txHash = isocoin::NULL_HASH;
      size_t blobSize = 0;
...
Crypto::Hash ICoreStub::getBlockIdByHeight(uint32_t height) {
  ...
  if (iter == blockHashByHeightIndex.end()) {
    return isocoin::NULL_HASH;
  }
  return iter->second;
}

bool ICoreStub::getBlockByHash(const Crypto::Hash &h, isocoin::Block &blk) {
...
void ICoreStub::getTransactions(const std::vector<Crypto::Hash>& txs_ids, std::list<isocoin::Transaction>& txs, std::list<Crypto::Hash>& missed_txs, bool checkTxPool) {
...
bool ICoreStub::scanOutputkeysForIndices(const isocoin::KeyInput& txInToKey, std::list<std::pair<Crypto::Hash, size_t>>& outputReferences) {
  return true;
}

bool ICoreStub::getBlockDifficulty(uint32_t height, isocoin::difficulty_type& difficulty) {
  return true;
}
...
bool ICoreStub::getBlockContainingTx(const Crypto::Hash& txId, Crypto::Hash& blockId, uint32_t& blockHeight) {
  ...
  blockHeight = boost::get<isocoin::BaseInput>(blockIter->second.baseTransaction.inputs.front()).blockIndex;
  return true;
}

bool ICoreStub::getMultisigOutputReference(const isocoin::MultisignatureInput& txInMultisig, std::pair<Crypto::Hash, size_t>& outputReference) {
  return true;
}

void ICoreStub::addBlock(const isocoin::Block& block) {
  uint32_t height = boost::get<isocoin::BaseInput>(block.baseTransaction.inputs.front()).blockIndex;
  Crypto::Hash hash = isocoin::get_block_hash(block);
  if (height > topHeight) {
    topHeight = height;
    topId = hash;
  }
  ...
  blockHashByTxHashIndex.emplace(std::make_pair(isocoin::getObjectHash(block.baseTransaction), hash));
  for (auto txHash : block.transactionHashes) {
    blockHashByTxHashIndex.emplace(std::make_pair(txHash, hash));
  }
}

void ICoreStub::addTransaction(const isocoin::Transaction& tx) {
  Crypto::Hash hash = isocoin::getObjectHash(tx);
...
bool ICoreStub::getOrphanBlocksByHeight(uint32_t height, std::vector<isocoin::Block>& blocks) {
  return true;
}

bool ICoreStub::getBlocksByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t blocksNumberLimit, std::vector<isocoin::Block>& blocks, uint32_t& blocksNumberWithinTimestamps) {
  return true;
}

bool ICoreStub::getPoolTransactionsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<isocoin::Transaction>& transactions, uint64_t& transactionsNumberWithinTimestamps) {
  return true;
}

bool ICoreStub::getTransactionsByPaymentId(const Crypto::Hash& paymentId, std::vector<isocoin::Transaction>& transactions) {
  return true;
}
...
std::unique_ptr<isocoin::IBlock> ICoreStub::getBlock(const Crypto::Hash& blockId) {
  return std::unique_ptr<isocoin::IBlock>(nullptr);
}

bool ICoreStub::handleIncomingTransaction(const isocoin::Transaction& tx, const Crypto::Hash& txHash, size_t blobSize, isocoin::tx_verification_context& tvc, bool keptByBlock) {
...
bool ICoreStub::addMessageQueue(isocoin::MessageQueue<isocoin::BlockchainMessage>& messageQueuePtr) {
  return true;
}

bool ICoreStub::removeMessageQueue(isocoin::MessageQueue<isocoin::BlockchainMessage>& messageQueuePtr) {
  return true;
}
