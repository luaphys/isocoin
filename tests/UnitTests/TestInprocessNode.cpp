...
namespace {
isocoin::Transaction createTx(isocoin::ITransactionReader& tx) {
  isocoin::Transaction outTx;
...
protected:
  ...
  ICoreStub coreStub;
  IisocoinProtocolQueryStub protocolQueryStub;
  isocoin::InProcessNode node;
...
TEST_F(InProcessNodeTests, initOk) {
  isocoin::InProcessNode newNode(coreStub, protocolQueryStub);
  CallbackStatus status;
...
TEST_F(InProcessNodeTests, getRandomOutsByAmountsSuccess) {
  ...
  isocoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response expectedResp;
  isocoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount out;
  out.amount = 10;
  out.outs.push_back({ 11, ignoredPublicKey });
  expectedResp.outs.push_back(out);
  coreStub.set_random_outs(expectedResp, true);

  std::vector<isocoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount> outs;
...
TEST_F(InProcessNodeTests, getNewBlocksUninitialized) {
  ...
  std::vector<isocoin::block_complete_entry> newBlocks;
...
TEST_F(InProcessNodeTests, relayTransactionUninitialized) {
  ...
  CallbackStatus status;
  newNode.relayTransaction(isocoin::Transaction(), [&] (std::error_code ec) { status.setStatus(ec); });
  ASSERT_TRUE(status.wait());
  ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getBlocksByHeightEmpty) {
  ...
  std::vector<std::vector<isocoin::BlockDetails>> blocks;
...
TEST_F(InProcessNodeTests, getBlocksByHeightMany) {
  ...
  std::vector<uint32_t> blockHeights;
  std::vector<std::vector<isocoin::BlockDetails>> actualBlocks;

  std::vector<isocoin::Block> expectedBlocks;
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    ...
    blockHeights.push_back(std::move(boost::get<isocoin::BaseInput>(iter->baseTransaction.inputs.front()).blockIndex));
  ...
  for (const boost::tuple<boost::tuple<size_t, isocoin::Block>, std::vector<isocoin::BlockDetails>>& sameHeight : range) {
    EXPECT_EQ(sameHeight.get<1>().size(), 1);
    for (const isocoin::BlockDetails& block : sameHeight.get<1>()) {
      EXPECT_EQ(block.height, sameHeight.get<0>().get<0>());
      Crypto::Hash expectedCryptoHash = isocoin::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getBlocksByHeightFail) {
  ...
  for (const isocoin::Block& block : generator.getBlockchain()) {
...
TEST_F(InProcessNodeTests, getBlocksByHashEmpty) {
  ...
  std::vector<isocoin::BlockDetails> blocks;
...
TEST_F(InProcessNodeTests, getBlocksByHashMany) {
  ...
  std::vector<Crypto::Hash> blockHashes;
  std::vector<isocoin::BlockDetails> actualBlocks;
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    expectedBlocks.push_back(*iter);
    blockHashes.push_back(isocoin::get_block_hash(*iter));
    coreStub.addBlock(*iter);
  }
  ...
  auto range = boost::combine(range1, actualBlocks);
  for (const boost::tuple<boost::tuple<Crypto::Hash, isocoin::Block>, isocoin::BlockDetails>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = isocoin::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getTxEmpty) {
  ...
  std::vector<isocoin::TransactionDetails> transactions;
...
TEST_F(InProcessNodeTests, getTxMany) {
  ...
  std::vector<Crypto::Hash> transactionHashes;
  std::vector<isocoin::TransactionDetails> actualTransactions;

  std::vector<std::tuple<isocoin::Transaction, Crypto::Hash, uint64_t>> expectedTransactions;

  coreStub.set_blockchain_top(0, boost::value_initialized<Crypto::Hash>());

  size_t prevBlockchainSize = generator.getBlockchain().size();
  for (size_t i = 0; i < BLOCKCHAIN_TX_NUMBER; ++i) {
    auto txptr = isocoin::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.push_back(isocoin::getObjectHash(tx));
    ...
    expectedTransactions.push_back(std::make_tuple(tx, isocoin::get_block_hash(generator.getBlockchain().back()), boost::get<isocoin::BaseInput>(generator.getBlockchain().back().baseTransaction.inputs.front()).blockIndex));
  }
  ...
  for (const boost::tuple<boost::tuple<Crypto::Hash, isocoin::TransactionDetails>, std::tuple<isocoin::Transaction, Crypto::Hash, uint64_t>>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = isocoin::getObjectHash(std::get<0>(sameHeight.get<1>()));
    EXPECT_EQ(expectedCryptoHash, sameHeight.get<0>().get<0>());
    Hash expectedHash = reinterpret_cast<const Hash&>(expectedCryptoHash);
...
TEST_F(InProcessNodeTests, getLastLocalBlockTimestamp) {
  ...
  isocoin::InProcessNode newNode(core, protocolQueryStub);
...
TEST_F(InProcessNodeTests, getPoolDiffereceActualBC) {
  ...
  for (size_t i = 0; i < POOL_TX_NUMBER; ++i) {
    auto txptr = isocoin::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.insert(isocoin::getObjectHash(tx));
    isocoin::tx_verification_context tvc = boost::value_initialized<tx_verification_context>();
    bool keptByBlock = false;
    coreStub.handleIncomingTransaction(tx, isocoin::getObjectHash(tx), isocoin::getObjectBinarySize(tx), tvc, keptByBlock);
  ...
  std::vector<Crypto::Hash> knownPoolTxIds;
  Crypto::Hash knownBlockId = isocoin::getObjectHash(generator.getBlockchain().back());
  bool isBcActual = false;
  std::vector<std::unique_ptr<ITransactionReader>> newTxs;
  std::vector<Crypto::Hash> deletedTxIds;
