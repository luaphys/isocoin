...
class TrivialWalletObserver : public isocoin::IWalletLegacyObserver
{
public:
...
struct SaveOnInitWalletObserver: public isocoin::IWalletLegacyObserver {
  SaveOnInitWalletObserver(isocoin::WalletLegacy* wallet) : wallet(wallet) {
  };
  ...
  isocoin::WalletLegacy* wallet;
  std::stringstream stream;
};

static const uint64_t TEST_BLOCK_REWARD = 70368744177663;

isocoin::TransactionId TransferMoney(isocoin::WalletLegacy& from, isocoin::WalletLegacy& to, int64_t amount, uint64_t fee, uint64_t mixIn = 0, const std::string& extra = "") {
  isocoin::WalletLegacyTransfer transfer;
...
{
public:
  WalletLegacyApi() : m_currency(isocoin::CurrencyBuilder(m_logger).currency()), generator(m_currency) {
  }
...
protected:
  ...
  void GetOneBlockReward(isocoin::WalletLegacy& wallet);
  void GetOneBlockReward(isocoin::WalletLegacy& wallet, const isocoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
  void GetOneBlockRewardAndUnlock(isocoin::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                  const isocoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
...
void WalletLegacyApi::prepareAliceWallet() {
  ...
  alice.reset(new isocoin::WalletLegacy(m_currency, *newNode));
  aliceNode = newNode;
...
void WalletLegacyApi::prepareBobWallet() {
  ...
  bob.reset(new isocoin::WalletLegacy(m_currency, *bobNode));
...
void WalletLegacyApi::prepareCarolWallet() {
  ...
  carol.reset(new isocoin::WalletLegacy(m_currency, *carolNode));
  carol->addObserver(carolWalletObserver.get());
}

void WalletLegacyApi::GetOneBlockReward(isocoin::WalletLegacy& wallet) {
  GetOneBlockReward(wallet, m_currency, generator);
}

void WalletLegacyApi::GetOneBlockReward(isocoin::WalletLegacy& wallet, const isocoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::GetOneBlockRewardAndUnlock(isocoin::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                                 const isocoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::performTransferWithErrorTx(const std::array<int64_t, 5>& amounts, uint64_t fee) {
  std::vector<isocoin::WalletLegacyTransfer> trs;
  isocoin::WalletLegacyTransfer tr;
...
TEST_F(WalletLegacyApi, TransactionsAndTransfersAfterSend) {
  ...
  isocoin::WalletLegacyTransaction tx;
...
TEST_F(WalletLegacyApi, getTransactionSuccess) {
  ...
  EXPECT_EQ(tx.firstTransferId, isocoin::WALLET_LEGACY_INVALID_TRANSFER_ID);
...
TEST_F(WalletLegacyApi, wrongPassword) {
  ...
  EXPECT_EQ(result.value(), isocoin::error::WRONG_PASSWORD);
}
...
TEST_F(WalletLegacyApi, sendMoneyToMyself) {
  ...
  isocoin::TransactionId txId = TransferMoney(*alice, *alice, 100000000, 100);
  ASSERT_NE(txId, isocoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, checkBalanceAfterSend) {
  ...
  isocoin::TransactionId txId = TransferMoney(*alice, *alice, sendAmount, fee);
  ASSERT_NE(txId, isocoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, moneyInPoolDontAffectActualBalance) {
  ...
  isocoin::TransactionId txId = TransferMoney(*alice, *bob, sendAmount, fee);
  ASSERT_NE(txId, isocoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  generator.generateEmptyBlocks(10);
...
TEST_F(WalletLegacyApi, initWithKeys) {
  isocoin::AccountKeys accountKeys;
...
TEST_F(WalletLegacyApi, sendAfterFailedTransaction) {
  ...
  isocoin::TransactionId txId = TransferMoney(*alice, *alice, 100000, 100);
  ASSERT_NE(txId, isocoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  alice->shutdown();
}
...
TEST_F(WalletLegacyApi, outcommingExternalTransactionTotalAmount) {
  class ExternalTxChecker : public isocoin::IWalletLegacyObserver {
  public:
    ExternalTxChecker(isocoin::WalletLegacy& wallet) : wallet(wallet), totalAmount(std::numeric_limits<int64_t>::max()) {
    }

    virtual void externalTransactionCreated(isocoin::TransactionId transactionId) override {
      isocoin::WalletLegacyTransaction txInfo;
  ...
  isocoin::WalletLegacy wallet(m_currency, *aliceNode);
...
namespace {
class WalletSynchronizationProgressUpdatedObserver : public isocoin::IWalletLegacyObserver {
public:
...
TEST_F(WalletLegacyApi, shutdownDoesNotRemoveObservers) {
  ...
  isocoin::WalletHelper::IWalletRemoveObserverGuard observerGuard(*alice, observer);
...
namespace {
class WalletTransactionEventCounter : public isocoin::IWalletLegacyObserver {
public:
  virtual void externalTransactionCreated(isocoin::TransactionId /*transactionId*/) override {
    ++m_count;
  }

  virtual void transactionUpdated(isocoin::TransactionId /*transactionId*/) override {
    ++m_count;
  }
...
TEST_F(WalletLegacyApi, afterShutdownAndInitWalletDoesNotSendNotificationsRelatedToOldAddress) {
  ...
  isocoin::AccountKeys accountKeys1;
  ...
  std::vector<isocoin::WalletLegacyTransfer> transfers;
...
TEST_F(WalletLegacyApi, resetDoesNotChangeAccountKeys) {
  ...
  isocoin::AccountKeys expectedAccountKeys;
  alice->getAccountKeys(expectedAccountKeys);

  alice->reset();

  isocoin::AccountKeys actualAccountKeys;
...
void generateWallet(isocoin::IWalletLegacy& wallet, TrivialWalletObserver& observer, const std::string& pass) {
...
TEST_F(WalletLegacyApi, outdatedUnconfirmedTransactionDeletedOnNewBlock) {
  ...
  isocoin::Currency currency(isocoin::CurrencyBuilder(m_logger).mempoolTxLiveTime(TRANSACTION_MEMPOOL_TIME).currency());
  TestBlockchainGenerator blockchainGenerator(currency);
  INodeTrivialRefreshStub node(blockchainGenerator);
  isocoin::WalletLegacy wallet(currency, node);
  TrivialWalletObserver walletObserver;
  wallet.addObserver(&walletObserver);
  ...
  isocoin::WalletLegacyTransaction transaction;
  ASSERT_TRUE(wallet.getTransaction(id, transaction));
  EXPECT_EQ(isocoin::WalletLegacyTransactionState::Deleted, transaction.state);
...
TEST_F(WalletLegacyApi, walletLoadsNullSpendSecretKey) {
  ...
  Crypto::generate_keys(accountKeys.address.spendPublicKey, accountKeys.spendSecretKey);
  Crypto::generate_keys(accountKeys.address.viewPublicKey, accountKeys.viewSecretKey);
  accountKeys.spendSecretKey = isocoin::NULL_SECRET_KEY;
