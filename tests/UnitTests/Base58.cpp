...
{
  isocoin::AccountPublicAddress addr;

  ASSERT_NO_THROW(isocoin::loadFromBinary(addr, Common::asBinaryArray(test_serialized_keys)));
  std::string addr_str = isocoin::getAccountAddressAsStr(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, addr);
...
TEST(parseAccountAddressString, handles_valid_address)
{
  uint64_t prefix;
  isocoin::AccountPublicAddress addr;
  ASSERT_TRUE(isocoin::parseAccountAddressString(prefix, addr, test_keys_addr_str));
  ASSERT_EQ(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, prefix);

  BinaryArray blob;
  ASSERT_NO_THROW(blob = isocoin::storeToBinary(addr));
...
TEST(parseAccountAddressString, fails_on_invalid_address_format)
{
  ...
  uint64_t prefix;
  isocoin::AccountPublicAddress addr;
  ASSERT_FALSE(isocoin::parseAccountAddressString(prefix, addr, addr_str));
}
