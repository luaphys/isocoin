...
#ifdef _WIN32
const std::string DAEMON_FILENAME = std::string(isocoin_NAME) + "d.exe";
#else
const std::string DAEMON_FILENAME = std::string(isocoin_NAME) + "d";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_" << isocoin_NAME << "d_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"" + std::string(isocoin_NAME) + "d" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), (std::string(isocoin_NAME) + "d").c_str(), "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
...
namespace {
  class WaitForCoinBaseObserver : public isocoin::IWalletLegacyObserver {
    Semaphore& m_gotReward;
    isocoin::IWalletLegacy& m_wallet;
  public:
    WaitForCoinBaseObserver(Semaphore& gotReward, isocoin::IWalletLegacy& wallet) : m_gotReward(gotReward), m_wallet(wallet) { }
    virtual void externalTransactionCreated(isocoin::TransactionId transactionId) override {
      isocoin::WalletLegacyTransaction trInfo;
...
bool BaseFunctionalTests::mineBlocks(TestNode& node, const isocoin::AccountPublicAddress& address, size_t blockCount) {
...
bool BaseFunctionalTests::prepareAndSubmitBlock(TestNode& node, isocoin::Block&& blockTemplate) {
  blockTemplate.timestamp = m_nextTimestamp;
  m_nextTimestamp += 2 * m_currency.difficultyTarget();

  BinaryArray blockBlob = isocoin::toBinaryArray(blockTemplate);
  return node.submitBlock(::Common::toHex(blockBlob.data(), blockBlob.size()));
}

bool BaseFunctionalTests::mineBlock(std::unique_ptr<isocoin::IWalletLegacy> &wallet) {
...
bool BaseFunctionalTests::makeWallet(std::unique_ptr<isocoin::IWalletLegacy> & wallet, std::unique_ptr<isocoin::INode>& node, const std::string& password) {
  if (!node) return false;
  wallet = std::unique_ptr<isocoin::IWalletLegacy>(new isocoin::WalletLegacy(m_currency, *node));
...
namespace {
  struct PeerCountWaiter : isocoin::INodeObserver {
...
bool BaseFunctionalTests::waitForPeerCount(isocoin::INode& node, size_t expectedPeerCount) {
...
bool BaseFunctionalTests::waitForPoolSize(size_t nodeIndex, isocoin::INode& node, size_t expectedPoolSize,
  std::vector<std::unique_ptr<isocoin::ITransactionReader>>& txPool) {
...
bool BaseFunctionalTests::getNodeTransactionPool(size_t nodeIndex, isocoin::INode& node,
  std::vector<std::unique_ptr<isocoin::ITransactionReader>>& txPool) {
