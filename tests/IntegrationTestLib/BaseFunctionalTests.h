...
namespace Tests {
  ...
  namespace po = boost::program_options;
    ...
    public:
      BaseFunctionalTests(const isocoin::Currency& currency, System::Dispatcher& d, const BaseFunctionalTestsConfig& config) :
          m_dispatcher(d),
          m_currency(currency),
    ...
    protected:
      ...
      bool makeWallet(std::unique_ptr<isocoin::IWalletLegacy> & wallet, std::unique_ptr<isocoin::INode>& node, const std::string& password = "pass");
      bool mineBlocks(TestNode& node, const isocoin::AccountPublicAddress& address, size_t blockCount);
      bool mineBlock(std::unique_ptr<isocoin::IWalletLegacy>& wallet);
      bool mineBlock();
      bool startMining(size_t threads);
      bool stopMining();

      bool getNodeTransactionPool(size_t nodeIndex, isocoin::INode& node, std::vector<std::unique_ptr<isocoin::ITransactionReader>>& txPool);

      bool waitDaemonsReady();
      bool waitDaemonReady(size_t nodeIndex);
      bool waitForPeerCount(isocoin::INode& node, size_t expectedPeerCount);
      bool waitForPoolSize(size_t nodeIndex, isocoin::INode& node, size_t expectedPoolSize,
        std::vector<std::unique_ptr<isocoin::ITransactionReader>>& txPool);

      bool prepareAndSubmitBlock(TestNode& node, isocoin::Block&& blockTemplate);
    ...
    private:
      ...
      std::unique_ptr<isocoin::INode> mainNode;
      std::unique_ptr<isocoin::IWalletLegacy> workingWallet;
