...
InProcTestNode::InProcTestNode(const TestNodeConfiguration& cfg, const isocoin::Currency& currency) : 
...
void InProcTestNode::workerThread(std::promise<std::string>& initPromise) {
  ...
  try {
    ...
    core.reset(new isocoin::core(m_currency, NULL, log));
    protocol.reset(new isocoin::isocoinProtocolHandler(m_currency, dispatcher, *core, NULL, log));
    p2pNode.reset(new isocoin::NodeServer(dispatcher, *protocol, log));
    protocol->set_p2p_endpoint(p2pNode.get());
    core->set_isocoin_protocol(protocol.get());

    isocoin::NetNodeConfig p2pConfig;
    ...
    isocoin::MinerConfig emptyMiner;
  ...
  core->deinit();
  p2pNode->deinit();
  core->set_isocoin_protocol(NULL);
...
bool InProcTestNode::getBlockTemplate(const std::string &minerAddress, isocoin::Block &blockTemplate, uint64_t &difficulty) {
  AccountPublicAddress addr;
...
bool InProcTestNode::makeINode(std::unique_ptr<isocoin::INode> &node) {

  std::unique_ptr<InProcessNode> inprocNode(new isocoin::InProcessNode(*core, *protocol));
