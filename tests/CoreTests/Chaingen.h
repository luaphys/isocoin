...
#include "isocoinCore/TransactionExtra.h"
...
struct serialized_object
{
  ...
  serialized_object(const isocoin::BinaryArray& a_data)
    : data(a_data)
  {
  }

  isocoin::BinaryArray data;
  //BEGIN_SERIALIZE_OBJECT()
  //  FIELD(data)
  //  END_SERIALIZE()
...
typedef serialized_object<isocoin::Block> serialized_block;
typedef serialized_object<isocoin::Transaction> serialized_transaction;
...
//VARIANT_TAG(binary_archive, isocoin::account_base, 0xcc);
//VARIANT_TAG(binary_archive, serialized_block, 0xcd);
//VARIANT_TAG(binary_archive, serialized_transaction, 0xce);
//VARIANT_TAG(binary_archive, event_visitor_settings, 0xcf);

typedef boost::variant<isocoin::Block, isocoin::Transaction, isocoin::AccountBase, callback_entry, serialized_block, serialized_transaction, event_visitor_settings> test_event_entry;
typedef std::unordered_map<Crypto::Hash, const isocoin::Transaction*> map_hash2tx_t;

class test_chain_unit_base: boost::noncopyable
{
public:
  test_chain_unit_base() :
    m_currency(isocoin::CurrencyBuilder(m_logger).currency()) {
  }

  typedef std::function<bool (isocoin::core& c, size_t ev_index, const std::vector<test_event_entry> &events)> verify_callback;
  typedef std::map<std::string, verify_callback> callbacks_map;

  const isocoin::Currency& currency() const;
  void register_callback(const std::string& cb_name, verify_callback cb);
  bool verify(const std::string& cb_name, isocoin::core& c, size_t ev_index, const std::vector<test_event_entry> &events);
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events, isocoin::Transaction& tx,
                         const isocoin::Block& blk_head, const isocoin::AccountBase& from, const isocoin::AccountBase& to,
                         uint64_t amount, uint64_t fee, size_t nmix);
isocoin::Transaction construct_tx_with_fee(Logging::ILogger& logger, std::vector<test_event_entry>& events, const isocoin::Block& blk_head,
                                            const isocoin::AccountBase& acc_from, const isocoin::AccountBase& acc_to,
                                            uint64_t amount, uint64_t fee);

void get_confirmed_txs(const std::vector<isocoin::Block>& blockchain, const map_hash2tx_t& mtx, map_hash2tx_t& confirmed_txs);
bool find_block_chain(const std::vector<test_event_entry>& events, std::vector<isocoin::Block>& blockchain, map_hash2tx_t& mtx, const Crypto::Hash& head);
void fill_tx_sources_and_destinations(const std::vector<test_event_entry>& events, const isocoin::Block& blk_head,
                                      const isocoin::AccountBase& from, const isocoin::AccountBase& to,
                                      uint64_t amount, uint64_t fee, size_t nmix,
                                      std::vector<isocoin::TransactionSourceEntry>& sources,
                                      std::vector<isocoin::TransactionDestinationEntry>& destinations);
uint64_t get_balance(const isocoin::AccountBase& addr, const std::vector<isocoin::Block>& blockchain, const map_hash2tx_t& mtx);

//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_tx_verification_context(const isocoin::tx_verification_context& tvc, bool tx_added, size_t event_index, const isocoin::Transaction& tx, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool do_check_tx_verification_context(const isocoin::tx_verification_context& tvc, bool tx_added, size_t /*event_index*/, const isocoin::Transaction& /*tx*/, t_test_class&, long)
{
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_tx_verification_context(const isocoin::tx_verification_context& tvc, bool tx_added, size_t event_index, const isocoin::Transaction& tx, t_test_class& validator)
{
  // SFINAE in action
...
//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_block_verification_context(const isocoin::block_verification_context& bvc, size_t event_index, const isocoin::Block& blk, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool do_check_block_verification_context(const isocoin::block_verification_context& bvc, size_t /*event_index*/, const isocoin::Block& /*blk*/, t_test_class&, long)
{
  // Default block verification context check
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_block_verification_context(const isocoin::block_verification_context& bvc, size_t event_index, const isocoin::Block& blk, t_test_class& validator)
{
  // SFINAE in action
...
public:
  push_core_event_visitor(isocoin::core& c, const std::vector<test_event_entry>& events, t_test_class& validator)
    : m_c(c)
    , m_events(events)
  ...
  bool operator()(const isocoin::Transaction& tx) const
  {
    log_event("isocoin::Transaction");
  ...
  bool operator()(const isocoin::Block& b) const
  {
    log_event("isocoin::Block");

    isocoin::block_verification_context bvc = boost::value_initialized<decltype(bvc)>();
  ...
  bool operator()(const isocoin::AccountBase& ab) const
  {
    log_event("isocoin::account_base");
    return true;
  }
  ...
  bool operator()(const serialized_block& sr_block) const
  {
    ...
    isocoin::Block blk;
    if (!isocoin::fromBinaryArray(blk, sr_block.data)) {
      blk = isocoin::Block();
    }
  ...
  bool operator()(const serialized_transaction& sr_tx) const
  {
    ...
    if (!isocoin::fromBinaryArray(tx, sr_tx.data)) {
      tx = isocoin::Transaction();
    }
...
//--------------------------------------------------------------------------
template<class t_test_class>
inline bool replay_events_through_core(isocoin::core& cr, const std::vector<test_event_entry>& events, t_test_class& validator)
{
  try {
    CHECK_AND_ASSERT_MES(typeid(isocoin::Block) == events[0].type(), false, "First event must be genesis block creation");
    cr.set_genesis_block(boost::get<isocoin::Block>(events[0]));
...
inline bool do_replay_events(std::vector<test_event_entry>& events, t_test_class& validator)
{
  ...
  isocoin::CoreConfig::initOptions(desc);
  ...
  isocoin::CoreConfig coreConfig;
  coreConfig.init(vm);
  isocoin::MinerConfig emptyMinerConfig;
  isocoin::isocoin_protocol_stub pr; //TODO: stub only for this kind of test, make real validation of relayed objects
  isocoin::core c(validator.currency(), &pr, logger);
...
#define MAKE_NEXT_BLOCK_TX1(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, TX1)         \
  isocoin::Block BLK_NAME;                                                         \
  {                                                                                   \
    std::list<isocoin::Transaction> tx_list;                                       \
...
#define REWIND_BLOCKS_N(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, COUNT)           \
  isocoin::Block BLK_NAME;                                                         \
  {                                                                                   \
    isocoin::Block blk_last = PREV_BLOCK;                                          \
...
#define MAKE_TX_LIST_START(VEC_EVENTS, SET_NAME, FROM, TO, AMOUNT, HEAD) \
    std::list<isocoin::Transaction> SET_NAME; \
...
const uint64_t COIN = Pow10<isocoin::parameters::isocoin_DISPLAY_DECIMAL_POINT>::value;
