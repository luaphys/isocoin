...
namespace isocoin
{
  ...
  struct P2pConnectionContext : public isocoinConnectionContext {
  public:
    ...
    P2pConnectionContext(P2pConnectionContext&& ctx) : 
      isocoinConnectionContext(std::move(ctx)),
      context(ctx.context),
      peerId(ctx.peerId),
  ...
  public:
    ...
    NodeServer(System::Dispatcher& dispatcher, isocoin::isocoinProtocolHandler& payload_handler, Logging::ILogger& log);
    ...
    isocoin::isocoinProtocolHandler& get_payload_object();
    ...
    isocoin::PeerlistManager& getPeerlistManager() { return m_peerlist; }
  ...
  private:
    ...
    bool handshake(isocoin::LevinProtocol& proto, P2pConnectionContext& context, bool just_take_peerlist = false);
    bool timedSync();
    ...
    virtual bool invoke_notify_to_peer(int command, const BinaryArray& req_buff, const isocoinConnectionContext& context) override;
    virtual void for_each_connection(std::function<void(isocoin::isocoinConnectionContext&, PeerIdType)> f) override;
    ...
    bool idle_worker();
    bool handle_remote_peerlist(const std::list<PeerlistEntry>& peerlist, time_t local_time, const isocoinConnectionContext& context);
    ...
    isocoinProtocolHandler& m_payload_handler;
