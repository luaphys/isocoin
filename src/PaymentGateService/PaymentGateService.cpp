...
const isocoin::Currency PaymentGateService::getCurrency() {
...
void PaymentGateService::runInProcess(Logging::LoggerRef& log) {
  ...
  isocoin::core core(currency, NULL, logger);

  isocoin::isocoinProtocolHandler protocol(currency, *dispatcher, core, NULL, logger);
  isocoin::NodeServer p2pNode(*dispatcher, protocol, logger);

  protocol.set_p2p_endpoint(&p2pNode);
  core.set_isocoin_protocol(&protocol);
  ...
  std::unique_ptr<isocoin::INode> node(new isocoin::InProcessNode(core, protocol));
...
void PaymentGateService::runRpcProxy(Logging::LoggerRef& log) {
  ...
  std::unique_ptr<isocoin::INode> node(
...
void PaymentGateService::runWalletService(const isocoin::Currency& currency, isocoin::INode& node) {
  PaymentService::WalletConfiguration walletConfiguration{
    config.gateConfiguration.containerFile,
    config.gateConfiguration.containerPassword
  };

  std::unique_ptr<isocoin::IWallet> wallet (WalletFactory::createWallet(currency, node, *dispatcher));
