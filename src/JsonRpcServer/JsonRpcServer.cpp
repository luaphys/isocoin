...
void JsonRpcServer::processRequest(const isocoin::HttpRequest& req, isocoin::HttpResponse& resp) {
  try {
    ...
    if (req.getUrl() == "/json_rpc") {
      ...
      try {
        ...
        resp.setStatus(isocoin::HttpResponse::STATUS_200);
      ...
      resp.setStatus(isocoin::HttpResponse::STATUS_404);
      return;
    }
  } catch (std::exception& e) {
    logger(Logging::WARNING) << "Error while processing http request: " << e.what();
    resp.setStatus(isocoin::HttpResponse::STATUS_500);
  }
}
