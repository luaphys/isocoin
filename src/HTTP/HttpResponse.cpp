...
const char* getStatusString(isocoin::HttpResponse::HTTP_STATUS status) {
  switch (status) {
  case isocoin::HttpResponse::STATUS_200:
    return "200 OK";
  case isocoin::HttpResponse::STATUS_404:
    return "404 Not Found";
  case isocoin::HttpResponse::STATUS_500:
...
const char* getErrorBody(isocoin::HttpResponse::HTTP_STATUS status) {
  switch (status) {
  case isocoin::HttpResponse::STATUS_404:
...
HttpResponse::HttpResponse() {
  status = STATUS_200;
  headers["Server"] = "isocoin-based HTTP server";
}
