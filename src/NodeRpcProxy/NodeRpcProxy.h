...
class NodeRpcProxy : public isocoin::INode {
public:
  ...
  virtual bool addObserver(isocoin::INodeRpcProxyObserver* observer);
  virtual bool removeObserver(isocoin::INodeRpcProxyObserver* observer);
...
private:
  ...
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>& knownBlockIds,
    std::vector<isocoin::block_complete_entry>& newBlocks, uint32_t& startHeight);
...
private:
  ...
  Tools::ObserverManager<isocoin::INodeObserver> m_observerManager;
  Tools::ObserverManager<isocoin::INodeRpcProxyObserver> m_rpcProxyObserverManager;
