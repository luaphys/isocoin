...
#include "isocoinCore/isocoinSerialization.h"
...
struct WalletTransactionDto {
  ...
  WalletTransactionDto(const isocoin::WalletTransaction& wallet) {
    state = wallet.state;
  ...
  isocoin::WalletTransactionState state;
  uint64_t timestamp;
  uint32_t blockHeight;
  Hash hash;
  int64_t totalAmount;
...
struct WalletTransferDto {
  ...
  WalletTransferDto(const isocoin::WalletTransfer& tr, uint32_t version) : WalletTransferDto(version) {
    address = tr.address;
    amount = tr.amount;
...
void serialize(WalletRecordDto& value, isocoin::ISerializer& serializer) {
...
void serialize(ObsoleteSpentOutputDto& value, isocoin::ISerializer& serializer) {
...
void serialize(ObsoleteChangeDto& value, isocoin::ISerializer& serializer) {
  serializer(value.txHash, "transaction_hash");
  serializer(value.amount, "amount");
}

void serialize(UnlockTransactionJobDto& value, isocoin::ISerializer& serializer) {
  serializer(value.blockHeight, "block_height");
  serializer(value.transactionHash, "transaction_hash");
  serializer(value.walletIndex, "wallet_index");
}

void serialize(WalletTransactionDto& value, isocoin::ISerializer& serializer) {
  typedef std::underlying_type<isocoin::WalletTransactionState>::type StateType;

  StateType state = static_cast<StateType>(value.state);
  serializer(state, "state");
  value.state = static_cast<isocoin::WalletTransactionState>(state);

  serializer(value.timestamp, "timestamp");
  isocoin::serializeBlockHeight(serializer, value.blockHeight, "block_height");
...
void serialize(WalletTransferDto& value, isocoin::ISerializer& serializer) {
...
std::string serialize(Object& obj, const std::string& name) {
  ...
  isocoin::BinaryOutputStreamSerializer s(output);
...
std::string encrypt(const std::string& plain, isocoin::CryptoContext& cryptoContext) {
  std::string cipher;
...
void addToStream(const std::string& cipher, const std::string& name, Common::IOutputStream& destination) {
  isocoin::BinaryOutputStreamSerializer s(destination);
  s(const_cast<std::string& >(cipher), name);
}

template<typename Object>
void serializeEncrypted(Object& obj, const std::string& name, isocoin::CryptoContext& cryptoContext, Common::IOutputStream& destination) {
...
std::string readCipher(Common::IInputStream& source, const std::string& name) {
  std::string cipher;
  isocoin::BinaryInputStreamSerializer s(source);
  s(cipher, name);

  return cipher;
}

std::string decrypt(const std::string& cipher, isocoin::CryptoContext& cryptoContext) {
  std::string plain;
...
void deserialize(Object& obj, const std::string& name, const std::string& plain) {
  ...
  isocoin::BinaryInputStreamSerializer s(stream);
  s(obj, Common::StringView(name));
}

template<typename Object>
void deserializeEncrypted(Object& obj, const std::string& name, isocoin::CryptoContext& cryptoContext, Common::IInputStream& source) {
...
void throwIfKeysMissmatch(const SecretKey& sec, const PublicKey& expected_pub) {
  if (!verifyKeys(sec, expected_pub))
    throw std::system_error(make_error_code(isocoin::error::WRONG_PASSWORD));
}

isocoin::WalletTransaction convert(const isocoin::WalletLegacyTransaction& tx) {
  isocoin::WalletTransaction mtx;

  mtx.state = isocoin::WalletTransactionState::SUCCEEDED;
...
isocoin::WalletTransfer convert(const isocoin::WalletLegacyTransfer& tr) {
  isocoin::WalletTransfer mtr;
...
void WalletSerializer::loadWallet(Common::IInputStream& source, const std::string& password, uint32_t version) {
  isocoin::CryptoContext cryptoContext;
...
void WalletSerializer::loadWalletV1(Common::IInputStream& source, const std::string& password) {
  ...
  isocoin::BinaryInputStreamSerializer encrypted(source);
  ...
  isocoin::BinaryInputStreamSerializer serializer(decryptedStream);
...
void WalletSerializer::loadWalletV1Keys(isocoin::BinaryInputStreamSerializer& serializer) {
  isocoin::KeysStorage keys;
...
void WalletSerializer::loadWalletV1Details(isocoin::BinaryInputStreamSerializer& serializer) {
...
void WalletSerializer::loadWallets(Common::IInputStream& source, CryptoContext& cryptoContext) {
  ...
  for (uint64_t i = 0; i < count; ++i) {
    ...
    wallet.container = reinterpret_cast<isocoin::ITransfersContainer*>(i); //dirty hack. container field must be unique
