...
struct keys_file_data {
  ...
  void serialize(isocoin::ISerializer& s) {
    s(iv, "iv");
...
void loadKeysFromFile(const std::string& filename, const std::string& password, isocoin::AccountBase& account) {
  keys_file_data keys_file_data;
  std::string buf;

  if (!Common::loadFileToString(filename, buf)) {
    throw std::system_error(make_error_code(isocoin::error::INTERNAL_WALLET_ERROR), "failed to load \"" + filename + '\"');
  }

  if (!isocoin::fromBinaryArray(keys_file_data, Common::asBinaryArray(buf))) {
    throw std::system_error(make_error_code(isocoin::error::INTERNAL_WALLET_ERROR), "failed to deserialize \"" + filename + '\"');
  }
  ...
  const isocoin::AccountKeys& keys = account.getAccountKeys();

  if (isocoin::loadFromBinaryKeyValue(account, account_data) &&
...
void importLegacyKeys(const std::string& legacyKeysFilename, const std::string& password, std::ostream& destination) {
  ...
  isocoin::WalletUserTransactionsCache transactionsCache;
  std::string cache;
  isocoin::WalletLegacySerializer importer(account, transactionsCache);
