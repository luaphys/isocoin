...
protected:
  ...
  struct ReceiverAmounts {
    isocoin::AccountPublicAddress receiver;
  ...
  void updateBalance(isocoin::ITransfersContainer* container);
  ...
  const WalletRecord& getWalletRecord(isocoin::ITransfersContainer* container) const;

  isocoin::AccountPublicAddress parseAddress(const std::string& address) const;
  ...
  void prepareTransaction(std::vector<WalletOuts>&& wallets,
    ...
    const isocoin::AccountPublicAddress& changeDestinationAddress,
  ...
  std::unique_ptr<isocoin::ITransaction> makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
    std::vector<InputInfo>& keysInfo, const std::string& extra, uint64_t unlockTimestamp);

  void sendTransaction(const isocoin::Transaction& isocoinTransaction);
  ...
  bool updateWalletTransactionInfo(size_t transactionId, const isocoin::TransactionInformation& info, int64_t totalAmount);
  ...
  void insertUnlockTransactionJob(const Crypto::Hash& transactionHash, uint32_t blockHeight, isocoin::ITransfersContainer* container);
  ...
  isocoin::AccountPublicAddress getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const;
