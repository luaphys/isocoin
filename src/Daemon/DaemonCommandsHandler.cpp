...
namespace {
  ...
  static bool print_as_json(const T& obj) {
    std::cout << isocoin::storeToJson(obj) << ENDL;
    return true;
  }
}


DaemonCommandsHandler::DaemonCommandsHandler(isocoin::core& core, isocoin::NodeServer& srv, Logging::LoggerManager& log) :
...
{
  std::stringstream ss;
  ss << isocoin::isocoin_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
  ss << "Commands: " << ENDL;
...
bool DaemonCommandsHandler::start_mining(const std::vector<std::string> &args) {
  ...
  isocoin::AccountPublicAddress adr;
