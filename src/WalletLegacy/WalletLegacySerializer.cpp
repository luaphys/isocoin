...
WalletLegacySerializer::WalletLegacySerializer(isocoin::AccountBase& account, WalletUserTransactionsCache& transactionsCache) :
  account(account),
...
void WalletLegacySerializer::serialize(std::ostream& stream, const std::string& password, bool saveDetailed, const std::string& cache) {
  ...
  isocoin::BinaryOutputStreamSerializer serializer(plainStream);
  saveKeys(serializer);
...
void WalletLegacySerializer::saveKeys(isocoin::ISerializer& serializer) {
  isocoin::KeysStorage keys;
  isocoin::AccountKeys acc = account.getAccountKeys();
...
void WalletLegacySerializer::deserialize(std::istream& stream, const std::string& password, std::string& cache) {
  ...
  isocoin::BinaryInputStreamSerializer serializerEncrypted(stdStream);
...
void WalletLegacySerializer::loadKeys(isocoin::ISerializer& serializer) {
  isocoin::KeysStorage keys;

  keys.serialize(serializer, "keys");

  isocoin::AccountKeys acc;
