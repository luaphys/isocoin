...
class PaymentServiceJsonRpcServer : public isocoin::JsonRpcServer {
public:
...
private:
  ...
  HandlerFunction jsonHandler(RequestHandler handler) {
    return [handler] (const Common::JsonValue& jsonRpcParams, Common::JsonValue& jsonResponse) mutable {
      ...
      try {
        isocoin::JsonInputValueSerializer inputSerializer(const_cast<Common::JsonValue&>(jsonRpcParams));
      ...
      isocoin::JsonOutputStreamSerializer outputSerializer;
