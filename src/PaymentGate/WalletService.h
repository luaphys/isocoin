...
class WalletService {
public:
  WalletService(const isocoin::Currency& currency, System::Dispatcher& sys, isocoin::INode& node, isocoin::IWallet& wallet, const WalletConfiguration& conf, Logging::ILogger& logger);
...
private:
  ...
  std::vector<isocoin::TransactionsInBlockInfo> getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const;
  std::vector<isocoin::TransactionsInBlockInfo> getTransactions(uint32_t firstBlockIndex, size_t blockCount) const;
