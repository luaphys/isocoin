This is the reference code for [isocoin](https://isocoin.org) cryptocurrency protocol.

* Launch your own isocoin currency: [isocoin Starter](https://isocoinstarter.org/)
* isocoin reference implementation: [isocoinCoin](https://isocoin-coin.org)
* Discussion board and support: [isocoin Forum](https://forum.isocoin.org)

## isocoin forking how-to

### Preparation

1. Create an account on [GitHub.com](github.com)
2. Fork [isocoin repository](https://github.com/isocoinfoundation/isocoin)
...
**1. in file src/isocoinConfig.h** - `isocoin_NAME` constant

Example: 
```
const char isocoin_NAME[] = "furiouscoin";
```
...
**1. Total money supply** (src/isocoinConfig.h)

Total amount of coins to be emitted. Most of isocoin based coins use `(uint64_t)(-1)` (equals to 18446744073709551616). You can define number explicitly (for example `UINT64_C(858986905600000000)`).
...
**2. Emission curve** (src/isocoinConfig.h)

Be default isocoin provides emission formula with slight decrease of block reward with each block. This is different from Bitcoin where block reward halves every 4 years.
...
**3. Difficulty target** (src/isocoinConfig.h)
...
In case you are not satisfied with isocoin default implementation of block reward logic you can also change it. The implementation is in `src/isocoinCore/Currency.cpp`:
```
...
- big block penalty calculation: this is the way isocoin protects the block chain from transaction flooding attacks and preserves opportunities for organic network growth at the same time.
...
**1. Default ports for P2P and RPC networking** (src/isocoinConfig.h)
...
```
const static boost::uuids::uuid isocoin_NETWORK = { { 0xA1, 0x1A, 0xA1, 0x1A, 0xA1, 0x0A, 0xA1, 0x0A, 0xA0, 0x1A, 0xA0, 0x1A, 0xA0, 0x1A, 0xA1, 0x1A } };
```


**3. Seed nodes** (src/isocoinConfig.h)
...
**1. Minimum transaction fee** (src/isocoinConfig.h)
...
**2. Penalty free block size** (src/isocoinConfig.h)

isocoin protects chain from tx flooding by reducing block reward for blocks larger than the median block size. However, this rule applies for blocks larger than `isocoin_BLOCK_GRANTED_FULL_REWARD_ZONE` bytes.

Example:
```
const size_t isocoin_BLOCK_GRANTED_FULL_REWARD_ZONE = 20000;
```


### Fifth step. Address prefix

You may choose a letter (in some cases several letters) all the coin's public addresses will start with. It is defined by `isocoin_PUBLIC_ADDRESS_BASE58_PREFIX` constant. Since the rules for address prefixes are nontrivial you may use the [prefix generator tool](https://isocoinstarter.org/tools.html).

Example:
```
const uint64_t isocoin_PUBLIC_ADDRESS_BASE58_PREFIX = 0xe9; // addresses start with "f"
```


### Sixth step. Genesis block

**1. Build the binaries with blank genesis tx hex** (src/isocoinConfig.h)
...
**3. Copy the printed transaction hash** (src/isocoinConfig.h)

Copy the tx hash that has been printed by the daemon to `GENESIS_COINBASE_TX_HEX` in `src/isocoinConfig.h`
